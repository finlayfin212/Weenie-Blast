function scr_player_jumphurt() {
	...
	if (player == 1)
	{
	    if ((global.player_hp <= 0) && (!instance_exists(obj_hand)))
	        instance_create((__view_get( e__VW.XView, 0 ) - 100), (__view_get( e__VW.YView, 0 ) + 300), obj_hand)
	}
	if (player == 2)
	{
	    if ((global.player2_hp <= 0) && (!instance_exists(obj_hand2)))
	        instance_create((__view_get( e__VW.XView, 0 ) - 100), (__view_get( e__VW.YView, 0 ) + 300), obj_hand2)
	}
	buffer_jump = 0
	image_alpha = 1
	inv_frames = 1
